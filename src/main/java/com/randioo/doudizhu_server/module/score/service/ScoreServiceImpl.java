...
public class ScoreServiceImpl extends ObserveBaseService implements ScoreService {
	...
	public void update(Observer observer, String msg, Object... args) {
		if (msg.equals(FightConstant.SEND_CARD)) {
			/*
			 * int gameId = (int) args[0]; String gameRoleId = (String) args[1];
	...
	public void updateScore(int gameId, String gameRoleId) {
		...
		int mark = (int) (game.getMultiple() * game.getGameConfig().getDi() * Math.pow(2, game.getMultiple()))
				* (landLordWin ? (game.isLandLordSpring() ? 2 : 1) : (game.isFarmerSpring() ? 2 : 1));
		System.out.println("Mark:" + mark + "Times:" + game.getMultiple());
		for (RoleGameInfo info : game.getRoleIdMap().values()) {
			info.currentMark = mark
					* (game.getLandlordGameRoleId().equals(info.gameRoleId) ? (landLordWin ? 2 : -2) : (landLordWin ? -1 : 1));
